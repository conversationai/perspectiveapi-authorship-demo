...
export class ConvaiChecker implements OnInit, OnChanges {
  ...
  private _handlePendingCheckRequest(text: string): Observable<AnalyzeCommentResponse> {
    ...
    this.pendingRequest$ = new Observable<AnalyzeCommentResponse>()
      .pipe(delay(REQUEST_LIMIT_MS))
      .pipe(() => { return this._checkText(text); })
    return return this._checkText(text)
  }
